...
public class ProductController {
	...
	ResponseEntity<List<String>> getAllCategories(){
		List<String> products = productService.getDistinctCategories();
		HttpHeaders headers = new HttpHeaders();
		headers.add("desc","All Categories");
		return ResponseEntity.ok().headers(headers).body(products);
	}
	@GetMapping("/products/dis-cat/{category}")
	ResponseEntity<List<TypeView>> getByCategoryDis(@PathVariable("category") String category){
		List<TypeView> products = productService.getDistinctTypeByCategory(category);
		HttpHeaders headers = new HttpHeaders();
		headers.add("desc","Get by choice");
		return ResponseEntity.ok().headers(headers).body(products);
	}

	@GetMapping("/products/type/{type}")
	ResponseEntity<List<Product>> getByType(@PathVariable("type") String type){
